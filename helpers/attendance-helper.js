...
module.exports = {
    ...
    getEmployeeDetails: (id) => {
        return new Promise(async (resolve, reject) => {
            ...
            db.get().collection(collections.EMPLOYEE_COLLECTION).findOne({ _id: ObjectId(id) }).then((data) => {
    ...
    setEmployeeAttendance: (data) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.ATTENDANCE_COLLECTION).insertOne(data).then((data) => {
                resolve(data)
            })
        })
    },
    ...
    setDutyShift: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            db.get().collection(collections.DUTY_SHIFT_COLLECTION).insertOne(data).then(async (data) => {
                resolve(data)
            })
        })
    },
    ...
    getMonthwiseAttendanceDetails: (id, targetmonth) => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //     }
            // ]).toArray()
            let monthData = await db.get().collection(collections.EMPLOYEE_ATTENDANCE_COLLECTION).aggregate([
                {
                    $project: {
                        _id: 1,
                        employeeId: 1,
            ...
            let attendanceData = await db.get().collection(collections.EMPLOYEE_ATTENDANCE_COLLECTION).aggregate([
                {
                    $project: {
                        _id: 1,
                        employeeId: 1,
    ...
    getEditingAttendance: (id) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.EMPLOYEE_ATTENDANCE_COLLECTION).findOne({ _id: ObjectId(id) }).then((data) => {
                resolve(data)
            })
        })
    },
    updateEmployeeAttendance: (data, id) => {
        ...
        return new Promise((resolve, reject) => {
            data.date = new Date(data.date)
            db.get().collection(collections.EMPLOYEE_ATTENDANCE_COLLECTION).updateOne({ _id: ObjectId(id) },
                {
                    $set: {
                        name: data.name,
                        date: data.date,
